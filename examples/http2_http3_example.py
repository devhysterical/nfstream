...
def example_http2_settings_analysis():
    ...
    for fp, data in settings_fingerprints.items():
        ...
        for param_id, value in data['params'].items():
            param_name = HTTP2Fingerprint.SETTINGS_PARAMS.get(param_id, f"ID_{param_id}")
...
def example_combined_analysis():
    ...
    print(f"  HTTP/2 Flows: {stats['http2']} ({stats['http2']/stats['total']*100:.1f}%)")
    print(f"  HTTP/3 Flows: {stats['http3']} ({stats['http3']/stats['total']*100:.1f}%)")
...
def example_export_to_csv():
    ...
    columns = [
        ...
        'udps.http2_detected', 'udps.http2_settings_fingerprint',
        'udps.http3_detected', 'udps.quic_version'
    ]
...
def example_client_identification():
    ...
    for client, fps in known_clients.items():
        unique_fps = set(fps)
        print(f"  {client}: {len(fps)} connections, {len(unique_fps)} unique fingerprints")
